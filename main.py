...
class DiskWiperGUI(ctk.CTk):
    ...
    def _setup_main_ui(self):
        ...
        for tag, color in [("info", config.COLOR_INFO), ("success", config.COLOR_SUCCESS), ("error", config.COLOR_CRITICAL), ("debug", "gray")]:
            self.log_box._textbox.tag_config(tag, foreground=color)

        footer = ctk.CTkFrame(self, fg_color="transparent")
        footer.pack(fill="x", side="bottom", pady=(0, 5), padx=20)
    ...
    def refresh(self):
        if self.refreshing or self.is_wiping: 
            return
        self.refreshing = True
        self.ref_btn.configure(state="disabled", text="Scanning...")
        if self.settings.get('debug'): 
            self.log("[DEBUG] Starting background disk scan...", "debug")
    ...
    def _rebuild_list(self, disks):
        ...
        if self.settings.get('debug'): 
            self.log(f"[DEBUG] Scan complete. Found {len(disks)} active disks.", "debug")

    def log(self, msg, level="default"): 
        if level == "debug" and not self.settings.get('debug', False):
            return 
    ...
    def _toggle_all(self):
        ...
        for cb, _ in self.checkboxes.values():
            if state and cb.cget("state") != "disabled": 
                cb.select()
            else: 
                cb.deselect()
